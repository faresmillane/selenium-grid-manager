...
const server = async () => {
    try {
        ...
        if(config["nodes"][list-1].active === true && platform === "macos"){
            await utils.shellExec(`sed -i -e 's/'max-sessions'=.*/'max-sessions'='${config["nodes"][list-1].sessions}'/g' ${config.path}config/${platform}/${config["nodes"][list-1].driverName}-node.toml`);
