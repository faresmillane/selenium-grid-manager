...
const updateDriver = async (driverName, url, platform) => {
    try {
        ...
        console.log(`download and extract ${driverName} from : ${url} ...`);
    }
    catch (error) {
        return false;
    };
...
const install = async () => {
    try {
        fs.existsSync(`${config.path}drivers`) ? exec(`sh ${config.path}lib/clear.sh "drivers/*" ${config.path}`) : fs.mkdirSync(`${config.path}drivers`);
        fs.existsSync(`${config.path}tmp`) ? exec(`sh ${config.path}lib/clear.sh "tmp/*" ${config.path}`) : fs.mkdirSync(`${config.path}tmp`);
        await updateDrivers();
    }
    catch (error) {
        return error;
